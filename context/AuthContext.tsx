...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const login = useCallback(async (user: string, password: string) => {
    console.log('AuthContext: Login attempt for user:', user);
    ...
    try {
      console.log('AuthContext: Setting AsyncStorage...');
      await AsyncStorage.setItem(STORAGE_KEY, user);
      console.log('AuthContext: AsyncStorage set successfully');
      
      console.log('AuthContext: Setting username state...');
      setUsername(user);
      console.log('AuthContext: Username state set to:', user);
    } catch (error) {
  ...
  const logout = useCallback(async () => {
    try {
      ...
      setUsername(null);
      console.log('AuthContext: Set username to null');
